...
const AddEditPlan: React.FC = () => {
    ...
    const { data: exercisePlanData, isLoading: exercisePlanLoading } = useGetRoutineConfigById(planId ?? "new");
    const { data: allExercisePlanData, isLoading: allExercisePlanLoading } = useGetRoutineConfigs(); // TODO: Dedicate an endpoint for these so we don't need to grab every routineConfig just to list injuries...
    const { data: allExerciseDetailData, isLoading: allExerciseDetailLoading } = useGetExerciseDetails();
    ...
    return (
        <>
            {(exercisePlanLoading || allExerciseDetailLoading || allExercisePlanLoading) && <Loader />}
